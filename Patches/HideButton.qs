...
function HideButton2(prefix) {
  ...
  //Step 1a - Find the address of the string
  var strAddr = exe.findString(prefix, RVA);
  if (strAddr === -1)
    return "Failed in Step 1 - Prefix missing";
  
  //Step 1b - Find its assignment to a stack location inside UIBasicWnd::OnCreate
  var offset = exe.findCode(strAddr.packToHex(4) + " C7", PTYPE_HEX, false);
  
  if (offset === -1)
   offset = exe.findCode(strAddr.packToHex(4) + " 8D", PTYPE_HEX, false);
 
  if (offset === -1)
    return "Failed in Step 1 - Prefix assignment missing";
