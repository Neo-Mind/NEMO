function RestoreRoulette() {  
  ...
  if (offset === -1)
    return "Failed in Part 1 - NUMACCOUNT not found";
  ...
  if (offset === -1)
    return "Failed in Part 1 - NUMACCOUNT reference missing";
...
//Sanity Check - Disable the patch if the icon bmp is not present. File name = 유저인터페이스\basic_interface\roullette\RoulletteIcon.bmp
