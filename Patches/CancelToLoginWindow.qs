...
function CancelToLoginWindow() {
  ...
  if (offset2 === -1)
    return "Failed in Step 1 - Callback case missing";
  
  //Step 1e - Check if there is a PUSH 118 before offset2. If yes the case begins with a PUSH 78 followed by PUSH 118 then the above code
  //          for 2013+ clients
  ...
  ;
  var offset3 = exe.find(code, PTYPE_HEX, true, "\xAB", offset2, offset2 + 40);
  
  if (offset3 === -1) {
    code = code.replace(" 3D AB 00 00 00", " 83 F8 AB");
    offset3 = exe.find(code, PTYPE_HEX, true, "\xAB", offset2, offset2 + 40);
  }
  
  if (offset3 === -1)
    return "Failed in Step 2 - MsgBox End missing";
  
  offset2 = offset3 + code.hexlength();
  ...
  offset3 = exe.find(code, PTYPE_HEX, false, "", offset2, offset2 + 30);
  if (offset3 === -1)
    return "Failed in Step 2 - Argument PUSH missing";
  ...
  if (offsetT === -1) {
    ...
    offsetT = exe.findCode(code, PTYPE_HEX, true, "\xAB");
  }
  ...
  code = ReplaceVarHex(code, 3, (offset3 + 2) - (offset + code.hexlength()-3));
