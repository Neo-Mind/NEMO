function EnableCustomShields() {
	...
	var max = 10;
	//Step 1 - Find first function insert location (function for storing the shield suffixes in memory) and accompanying values needed
	//1.1 - Locate _°¡µå (Guard's suffix)
	var offset = exe.findString("\x5F\xB0\xA1\xB5\xE5", RVA);
	if (offset == -1) {
		return "Failed in Step 1.1";
	}
	
	//1.2 - Find location where it is referenced (moved to memory location)
	...
	//1.3 - Insert location comes 18 bytes before offset.
	var insReq = offset - 18;
	
	//1.4 - Extract Default String used (Null byte starting)
	var zeroS = exe.fetchDWord(insReq + 8);

	//Step 2 - Find second function insert location (function for mapping storage id from item id)
	//2.1 - Find location where the original mapping function is called
	code = " 3D D0 07 00 00 7E AB 50 B9 AB AB AB AB E8 AB AB AB AB 89 86";
	offset = exe.findCode(code, PTYPE_HEX, true, "\xAB");
	if (offset == -1) {
		return "Failed in Step 2.1";
	}
	
	//2.2 - Function is called after 13 bytes from offset (including call opcode = 14)
	var insMap = offset + 13 + 5 + exe.fetchDWord(offset + 14);
	
	//Step 3 - Find offsets & function locations for using in lua function
	//3.1 - Find offset of ReqJobName & ReqAccName
	offset = exe.findString("ReqJobName", RVA);
	if (offset == -1) {	
		return "Failed in Step 3.1-a";
	}
	
	var offset2 = exe.findString("ReqAccName", RVA);
	if (offset2 == -1) {
		return "Failed in Step 3.1-b";
	}
	
	//3.2 - Find where they are referenced
	offset = exe.findCode(" 68" + offset.packToHex(4), PTYPE_HEX, " ");
	if (offset == -1) {
		return "Failed in Step 3.2-a";
	}
	
	offset2 = exe.findCode(" 68" + offset2.packToHex(4), PTYPE_HEX, " ");
	if (offset2 == -1) {
		return "Failed in Step 3.2-b";
	}		
	
	//3.3 - All values are x bytes from pushed location - x varies based on compiler version
	...
	code = code + " 68 00 00 00 00";	//PUSH OFFSET GetShieldID
	...
	if (exe.getClientDate() > 20130605) {
		...
		code = code + " FF 15" + stringAlloc.packToHex(4);	//CALL DWORD PTR DS:[FN01] - String allocator
		var fn1loc1 = false;
	}
	...
	code = code + " 68 00 00 00 00";	//PUSH OFFSET ReqShieldName
	...
	//4.4 - Special for 2013 clients - remove item id limitation.
	...
	//Step 5 - Create Lua Reader code
	//5.1 - Find offset of Lua Files\Effect\NewEffect_F
	var newEff = exe.findString("Lua Files\\DataInfo\\NPCIdentity", RVA);
	if (newEff == false) {
		return "Failed in Step 5.1";
	}
	
	//5.2 - Find pushed location
	offset = exe.findCode("68" + newEff.packToHex(4), PTYPE_HEX, false);
	if (offset == -1) {
		return "Failed in Step 5.2";
	}
	
	//5.3 - Function to call for reading file
	var lualoader = offset + 5 + 5 + exe.fetchDWord(offset + 6);
	
	//5.4 - Return jmp location
	var jmpback = offset + 10;
	
	//5.5 - Read out prefix to use before pushing file basename
	...
	//7.1 - jmper has to jump to insertLocation
	...
	//7.8 - Loop completion for Req	
	code = code.replaceAt(loopLoc, ((ReqBegin/3 + 9) - (loopLoc/3 + 1)).packToHex(1) );	
	
	//7.9 - 2013 Special (specify call location)
	if (special > 0) {
	...
	//Step 9 - By this point Lua File reading is proper now we need to replace the other two functions
	// 9.1 - At insReq we have MOV E*X, [ESI+####] ,we make it a known register first - EAX then PUSH it and call the function and skip the allocations below
	...
	// 9.2 - Modify the hardcoded limits used for storage allocation
	...
	if (offset == -1) {
		return "Failed in step 9.2-b";
	}
	exe.replace(offset, (max+1).packToHex(1), PTYPE_HEX);
	
	// 9.3 - For insMap we are going to add a wrapper function - i dunno why it makes sense
	code = 	  " 55"				//PUSH EBP
			+ " 8B EC"			//MOV EBP,ESP
			+ " 8B 45 08"		//MOV EAX,DWORD PTR SS:[EBP+8]
			+ " 50"				//PUSH EAX
