function EnableTitleBarMenu() {
  ...
  if (offsets.length === 0)
    return "Failed in Part 1 - Style not found";
 
  //Step 1c - Find which one precedes Function call
  code = " FF 15" + func.packToHex(4); //CALL DWORD PTR DS:[<&USER32.CreateWindowExA>]
  for (var i = 0; i < offsets.length; i++) {
    var offset = exe.find(code, PTYPE_HEX, false, "", offsets[i]+8, offsets[i]+29);//5 + 3 for minimum operand pushes, 5 + 18 for maximum operand pushes + 6 for function call
    if (offset !== -1) {
  ...
  if (offset === -1)
    return "Failed in Part 1 - Function call not found";
